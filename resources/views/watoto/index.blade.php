        ...
        <div class="grid lg:grid-cols-2 gap-8 items-stretch">
            ...
            <div class="group relative bg-white rounded-[2rem] overflow-hidden shadow-xl hover:shadow-2xl transition">
                <iframe
                    ...
                    src="{{ $featuredVideo->youtube_embed }}"
            ...
            <div class="relative bg-white rounded-[2rem] p-8 md:p-10 shadow-xl border border-gray-100 flex flex-col justify-center">
                ...
                <div class="space-y-6 relative z-10">
                    ...
                    @if($featuredVideo->main_lesson)
                        <div>
                            <h3 class="font-bold text-lg text-[#0E3D4F]">Funzo Kuu</h3>
                            <p class="text-gray-600 mt-1">{{ $featuredVideo->main_lesson }}</p>
                        </div>
                    @endif

                    @if($featuredVideo->bible_verse)
                        <div>
                            <h3 class="font-bold text-lg text-[#0E3D4F]">Mstari wa Biblia</h3>
                            <p class="text-gray-600 mt-1">{{ $featuredVideo->bible_verse }}</p>
                        </div>
                    @endif

                    @if($featuredVideo->reflection_question)
                        <div>
                            <h3 class="font-bold text-lg text-[#0E3D4F]">Swali la Kutafakari</h3>
                            <p class="text-gray-600 mt-1">{{ $featuredVideo->reflection_question }}</p>
                        </div>
                    @endif

                    <a href="{{ route('children.show', $featuredVideo->slug) }}"
        ...
        @if($videos->count())
            <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
                @foreach($videos as $video)
                    <article id="{{ $video->category ? Str::slug($video->category) : 'video-'.$video->id }}"
                        ...
                        <a href="{{ route('children.show', $video->slug) }}" class="relative block overflow-hidden">
                        ...
                        <div class="p-6">
                            ...
                            <h3 class="text-xl font-black text-[#0E3D4F] mb-4 leading-snug">
                                <a href="{{ route('children.show', $video->slug) }}" class="hover:text-[#0083CB] transition">
                                    {{ $video->title }}
                                </a>
                            </h3>
