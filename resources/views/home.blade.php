...
@section('title', 'Uzima Milele')

@section('content')

{{-- HERO (Image + Text Slider Combined) --}}
<section
    x-data="{
        active: 0,
        slides: [
            {
                ...
                btn1: 'Soma Tafakari',
                link1: '{{ route('devotions.index') }}',
                btn2: 'Anza Masomo',
                link2: '{{ route('lessons.index') }}'
            },
            {
            ...
            },
            {
                ...
                btn1: 'Tuma Ombi',
                link1: '{{ route('prayers.testimonies') }}',
                btn2: 'Soma Tafakari',
                link2: '{{ route('devotions.index') }}'
            }
        ]
    }"
...
<section class="bg-gray-50 py-16 md:py-24">
    <div class="max-w-7xl mx-auto px-4">
        ...
        @if(isset($latestDevotions) && $latestDevotions->count())
            <div class="grid md:grid-cols-3 gap-7">
                @foreach($latestDevotions->take(3) as $devotion)
                    <a href="{{ route('devotions.show', $devotion->slug) }}"
                       class="group bg-white rounded-[2rem] overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition">
                        <div class="h-56 overflow-hidden bg-gray-100">
                            <img src="{{ $devotion->image ? asset('storage/' . $devotion->image) : asset('images/devotion-default.jpg') }}"
                                 alt="{{ $devotion->title }}"
                        ...
                        <div class="p-6">
                            <p class="text-sm text-primary font-black mb-3">
                                {{ $devotion->published_at ? \Carbon\Carbon::parse($devotion->published_at)->format('d M Y') : 'Tafakari' }}
                            </p>
...
<section class="bg-white py-16 md:py-24">
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            <div class="rounded-[2rem] overflow-hidden shadow-xl bg-gray-100">
                <img src="{{ asset('images/biblestudy.jpg') }}"
    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="grid lg:grid-cols-2 gap-12 items-center">
            ...
            <div class="bg-white/10 rounded-[2rem] p-4 border border-white/10">
                <img src="{{ asset('images/childbiblestudy.jpg') }}"
