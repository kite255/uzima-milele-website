...
class LessonEnrollmentController extends Controller
{
    public function store(Request $request, Lesson $lesson)
    {
        ...
        if (! $user) {
            return redirect()
                ->route('login', [
                    'redirect' => route('lessons.show', $lesson->slug),
                ])
                ->with(
                    'error',
                    'Tafadhali ingia kwanza ili kujiunga na somo.'
                );
        }
        ...
        $data = $request->validate([
            'study_pace' => [
                'nullable',
                'in:relaxed,regular,intensive,custom',
            ],
        ...
        */
        ...
        $targetCompletionDate =
            $lesson->calculateTargetCompletionDate(
                pace: $studyPace,
                customHours: $customHours
            );
        ...
        */
        ...
        if (
            ...
            return redirect()
                ...
                ->with(
                    'info',
                    'Tayari umejiunga na somo hili. Kubadilisha ratiba hakujaruhusiwa kwa somo hili.'
                );
        }
        ...
        */
        $enrollment->forceFill([
            ...
            'schedule_started_at' =>
                $enrollment->schedule_started_at
                ?: now(),
        ...
        /*
        |--------------------------------------------------------------------------
        | Assign Follow-up Instructor
        |--------------------------------------------------------------------------
        |
        | Only a brand-new enrollment is assigned automatically.
        |
        | Existing enrollments keep their current follow-up instructor when
        | the student changes or resets their learning schedule.
        |
        | If the course has no eligible instructor, enrollment continues
        | normally and the student remains unassigned until an administrator
        | assigns someone later.
        |
        */
        if (
            $wasNewEnrollment
        ...
        */
        if ($wasNewEnrollment) {
            ...
            return redirect()
                ...
                ->with(
                    'success',
                    'Umejiunga na somo kikamilifu na ratiba yako ya kujifunza imeandaliwa.'
                );
        }

        /*
        |--------------------------------------------------------------------------
        | Existing Enrollment Response
        |--------------------------------------------------------------------------
        */
        return redirect()
            ->route(
            ...
            ->with(
                'info',
                'Ratiba yako ya kujifunza imesasishwa.'
            );
    }
}
